    ...
    public class StaffController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (responseMessage.IsSuccessStatusCode) //adresten başarılı bir istek dönerse
            {
                ...
                var values = JsonConvert.DeserializeObject<List<StaffViewModel>>(jsonData); //gelen json veriyi deseralize ederek okunur hale getirdim
                return View(values);
            }
        ...
        public async Task<IActionResult> AddStaff(AddStaffViewModel model)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:60431/api/Staff", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:60431/api/Staff/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:60431/api/Staff/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateStaff(UpdateStaffViewModel model)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:60431/api/Staff/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
